...
# replication:
odir = "/cbica/projects/csdsi/replication/data/figs/streamlines_corr_fullDSI-rel/"+grp+"/"
os.makedirs(odir, exist_ok=True)

# output folder for csv:
# hamsi's:
# odir_csv = "/cbica/projects/csdsi/cleaned_paper_analysis/bug_fix/data/dice_scores/"+grp+"/"
# replication:
odir_csv = "/cbica/projects/csdsi/replication/data/dice_scores/"+grp+"/"
os.makedirs(odir_csv, exist_ok=True)
...
def get_median_frame():
    ...
    median_df.to_csv(odir_csv+mtrk+"_medians.csv")
...
def get_stats_and_linregress(median_df):
    ...
    r_df.to_csv(odir_csv+mtrk+"_median_tracks_slopes_with_reliability.csv")
    return fit_df
...
def main():
    if os.path.exists(odir_csv+mtrk+"_mediansa.csv") == False:
        get_median_frame()
    median_df= pd.read_csv(odir_csv+mtrk+"_medians.csv")
