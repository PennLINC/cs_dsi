...
def make_violin_friendly_frames(trk):
    ...
    # replication:
    comb_df.to_csv(indir_tosave+"data_violin-friendly.csv")
    ...
    # CZ notes: when AF_L, create a csv; else tracks, append to this csv
    if trk == "Arcuate_Fasciculus_L":
        sane_df.to_csv(folder_sanity_check+grp+"_tidydata_trackwise.csv", index=False)
    else:
        app_df = pd.read_csv(folder_sanity_check+grp+"_tidydata_trackwise.csv")
        app_df = app_df.append(sane_df)
        app_df.to_csv(folder_sanity_check+grp+"_tidydata_trackwise.csv", index=False)
